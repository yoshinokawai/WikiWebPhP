...
<main class="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 mb-8">
        ...
        <a href="<?php echo get_post_type_archive_link('vtuber_wiki'); ?>" class="hover:text-primary transition-colors">VTuber Wiki</a>
        <span class="material-symbols-outlined text-[16px]">chevron_right</span>
            ...
            <div class="lg:col-span-7 p-8 lg:p-12">
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6 pt-6 border-t border-slate-200 dark:border-slate-800">
                        <div>
                            <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide mb-1">Debut Date</p>
                            <p class="text-xl font-black text-slate-900 dark:text-white"><?php echo $debut_date ? date('M d, Y', strtotime($debut_date)) : 'N/A'; ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide mb-1">Status</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <article class="lg:col-span-8 space-y-8">
            <section>
                <h2 class="text-2xl font-black text-slate-900 dark:text-white mb-6 border-b-4 border-primary/20 inline-block pb-1">Tiểu sử & Lore</h2>
                <div class="prose prose-slate dark:prose-invert max-w-none text-lg text-slate-600 dark:text-slate-300 leading-relaxed">
                    ...
                    <?php else : ?>
                        <p class="italic text-slate-400">Tiểu sử của nhân vật này đang được cập nhật...</p>
                    <?php endif; ?>
                </div>
            </section>

            <section>
                <h2 class="text-2xl font-black text-slate-900 dark:text-white mb-6 border-b-4 border-primary/20 inline-block pb-1">Video & Nội dung khác</h2>
                <div class="aspect-video rounded-3xl overflow-hidden bg-slate-900 shadow-xl">
                    ...
                    <div class="w-full h-full flex flex-col items-center justify-center text-white/50 space-y-3">
                         ...
                         <p class="font-bold">YouTube Content Coming Soon</p>
                    </div>
                </div>
            </section>
        </article>
        ...
        <aside class="lg:col-span-4 space-y-8">
            <section class="bg-white dark:bg-surface-dark p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-soft">
                ...
                <div class="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <?php else : ?>
                        <div class="text-center py-8 text-slate-400">
                            <p class="text-sm italic">Chưa có lịch sử donate cho VTuber này.</p>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <button class="w-full mt-6 h-12 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold hover:scale-[1.02] transition-transform">
                    Donate cho <?php the_title(); ?>
                </button>
            </section>

            <section class="bg-gradient-to-br from-indigo-600 to-primary p-6 rounded-3xl text-white shadow-lg shadow-primary/20">
                <h4 class="font-bold mb-2">Trở thành cộng tác viên</h4>
                <p class="text-sm text-white/80 mb-4">Giúp chúng tôi hoàn thiện thông tin về <?php the_title(); ?> bằng cách đóng góp nội dung mới.</p>
                <button class="w-full h-10 rounded-lg bg-white/20 hover:bg-white/30 transition-colors font-bold text-sm">Chỉnh sửa hồ sơ</button>
            </section>
        </aside>
    </div>
</main>
